...
void addalienbullets(void)
{
	...
	do
	{
		...
		&& (gv.nbullets < MAXBULLETS - 16))				//leave some bullets for the player
		{
...
void runspecial(void)
{
	switch(gv.player.special)
	{
		...
		{
			if((!gv.nbullets)
			|| ((gv.bullets[0].type & BTMASK) != bullet_bomb))
			{
				//check if the bomb was destroyed
				if(gv.player.specialdata >= 32)
				{
					gv.player.specialdata--;
				}
				else if(!gv.player.specialdata)
				{
			...
			{
				//targeti/j really shouldn't be used for this...
				gv.player.targetj = gv.bullets[0].y>>FRACBITS;
				gv.player.targeti = gv.bullets[0].x>>FRACBITS;

				if((gv.player.targetj <= (gv.alieny>>FRACBITS) + (gv.abottombound+1)*(ALIENHEIGHT/2))
				|| (gv.player.targetj < 32))
				{
					//once it reaches the target range, blow it up
					removebullet(0);
				}
			}
			...
			if(gv.player.specialdata < 32)
			{
				if((gv.player.specialdata & 7) == 1)
				{
					addeffect(gv.player.targeti, gv.player.targetj, effect_explode);
					setsound(sound_firewar);
				}

				addbullet(gv.player.targeti,
					gv.player.targetj,
					makeplayerbullet(bullet_star),
					(gv.player.specialdata--));
			}
